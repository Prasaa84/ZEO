...
?>
  <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <?php
              $status = ''; // ගුරු ආයතන අංශයට අළුතින් ලැබුනු ප්‍රමාණය 
              $ci = & get_instance();
      ...
      <?php
              $status = ''; // ගුරු ආයතන අංශයනට භාරදෙන ලද මුලු වැටුප් වර්ධක ප්‍රමාණය  
              $ci = & get_instance();
              $ci->load->model('Increment_model');
              $condition = '(tit.inc_status_id=3 or tit.inc_status_id=4 or tit.inc_status_id=5) and tit.increment_year='.date("Y");
              $received = $ci->Increment_model->count_increment_by_condition($condition);
              echo '<h1 align="center">'.$received.'</h1>';
      ?>
            </div>
      ...
      ?>
            ...
            <a class="card-footer text-white clearfix small z-1" href="<?php echo base_url(); ?>Increment/viewIncrements/3/1">
              <span class="float-left"> View... </span>
              <span class="float-right">
                <i class="fa fa-angle-right"></i>
              </span>
      ...
      <?php
              $status = 2; // ගුරු ආයතන අංශයෙන් අනුමත කරන ලදි  
              $ci = & get_instance();
              $ci->load->model('Increment_model');
              $condition = '( tit.inc_status_id=4 or tit.inc_status_id=5 ) and tit.increment_year='.date("Y");
              $approved = $ci->Increment_model->count_increment_by_condition($condition);
              echo '<h1 align="center">'.$approved.'</h1>';
      ?>
            </div>
            <a class="card-footer text-white clearfix small z-1" href="<?php echo base_url(); ?>Increment/viewIncrements/4">
              <span class="float-left"> View... </span>
              <span class="float-right">
                <i class="fa fa-angle-right"></i>
              </span>
      ...
      <?php
              $ci = & get_instance();             // වැටුප් අංශයට භාර දුන් ප්‍රමාණය 
              $ci->load->model('Increment_model');
              $condition = '( tit.inc_status_id=5 ) and tit.increment_year='.date("Y");
              $submitted = $ci->Increment_model->count_increment_by_condition($condition);
              echo '<h1 align="center">'.$submitted.'</h1>';
      ?>
            </div>
            <a class="card-footer text-white clearfix small z-1" href="<?php echo base_url(); ?>Increment/viewIncrements/5">
              <span class="float-left"> View... </span>
              <span class="float-right">
                <i class="fa fa-angle-right"></i>
              </span>

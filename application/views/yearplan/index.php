<style type="text/css">
  ...
  .deleteEvent:hover{text-decoration: underline;}
  .deleteEvent{color:#0000ff;}
</style>
<div class="content-wrapper">
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item">
          <a href="<?php echo base_url(); ?>YearPlan">Events of Year Plan </a>
        </li>
      </ol>
      <?php
      ...
      <div class="row">     
        ...
        <?php if(empty($events)){ ?>
                <div class="alert alert-danger"> No recent events found  </div>
        <?php }else{
                foreach ($events as $event) { $event_id = $event->id; ?>
                    <div class="news<?php echo $event_id; ?>">
                        <h4><?php echo $event->title; ?></h4>
                        <p><?php echo $event->description; ?></p>
                <?php   $added_dt_tm = strtotime($event->date_added);
                        $added_dt = date("j F Y",$added_dt_tm);
                        $added_tm = date("h:i A",$added_dt_tm);
                ?>
                        <p><small><?php echo 'Added on '.$added_dt.' at '.$added_tm; ?></small>
                    <?php 
                        // echo $this->session->userdata['userrole'];
                        if($this->session->userdata['userrole'] == 'System Administrator'){
                          echo ' <small><a href="'.base_url().'YearPlan/eventUpdateView/'.$event_id.'"> Edit </a> or <a href="" class="deleteEvent" data-id="'.$event_id.'"> Remove </a></small>';
                        }
                    ?>
                        </p>
                    </div>
        ...
        ?>
          ...
          <?php   if(!empty($eventStartDate)){ ?>
              ...
              <?php   foreach ($eventStartDate as $eventStartDate) {
                          $added_dt_tm = strtotime($eventStartDate->start_event);
                          $year_month = date("Y F",$added_dt_tm);  // month name
                          $year_month2 = date("Y-m",$added_dt_tm); // month number
                          echo '<li><a href="'.base_url().'YearPlan/viewEventsByMonth/'.$year_month2.' ">'.$year_month.'</a></li>'; 
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLabel"><i class="fa fa-building"></i> Event ඇතුළත් කිරීම</h5>
           ...
           <?php
                  $attributes = array("class" => "form-horizontal", "id" => "add_event_form", "name" => "add_event_form", "accept-charset" => "UTF-8" );
                  echo form_open_multipart("YearPlan/addEvent", $attributes);?>
                  <fieldset>
                    ...
                    <div class="form-group">
                      <div class="row">
                        ...
                        <div class="col-lg-9 col-sm-9">
                          <textarea class="form-control" id="text_txtarea" name="text_txtarea" rows="4"></textarea>
                        </div>
                      </div> <!-- /row -->
                    </div> <!-- /form group -->
                    <div class="form-group">
                      <div class="row">
                        <div class="col-lg-3 col-sm-3">
                          <label for="start event">ආරම්භක දිනය </label>
                        </div>
                        <div class="col-lg-9 col-sm-9">
                          <input type="text" name="start_event_txt" class="form-control datepicker">
                        </div>
                      </div> <!-- /row -->
                    </div> <!-- /form group -->
                    <div class="form-group">
                      <div class="row">
                        <div class="col-lg-3 col-sm-3">
                          <label for="end event">අවසන් වන දිනය </label>
                        </div>
                        <div class="col-lg-9 col-sm-9">
                          <input type="text" name="end_event_txt" class="form-control datepicker">
                        </div>
                      </div> <!-- /row -->
                    </div> <!-- /form group -->
                  <!-- <fieldset> -->
...
<script type="text/javascript">
  ...
  $(".deleteEvent").click(function(e){
      ...
      function(isConfirm) {
        if (isConfirm) {
          $.ajax({
            url:"<?php echo base_url(); ?>YearPlan/deleteEvent",  
            method:"POST",  
            ...
            error: function() {
             ...
             success: function(data) {
                  if(data.trim()=='true'){
                    ...
                    swal("Error!", "Event not deleted.", "error");
                  }
             }
          });
        } 

...
?>
  <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">List of Students on Marks</li>
      </ol>
      <?php
                  ...
                  <?php $attributes = array("class" => "form-inline", "id" => "import_marks_form", "name" => "import_marks_form", "role" => "search", "enctype" => "multipart/form-data");
                    ...
                    <?php } ?>
                          ...
                          <div class="row">
                            ...
                            <div class="col-lg-1 col-sm-1">
                              <select class="form-control form-control-sm mb-1" id="subject_select" name="subject_select" data-toggle="tooltip" title="Select a Subject">
                                <option value="" selected>------</option>
                              </select>
                              <span class="text-danger" id="class_error"><?php echo form_error('subject_select'); ?></span>
                            </div>
                          </div> <!-- /row -->
                          <div class="row">
                            <div class="col-lg-1 col-sm-1">
                            ...
                            <div class="col-lg-1 col-sm-1">
                              <input type="text" name="from_txt" id="from_txt" class="form-control form-control-sm" data-toggle="tooltip" title="Enter marks" size="2">
                              <span class="text-danger" id="class_error"><?php echo form_error('from_txt'); ?></span>
                            </div>
                          </div> <!-- /row -->
                          <div class="row">
                            <div class="col-lg-1 col-sm-1">
                            ...
                            <div class="col-lg-1 col-sm-1">
                              <input type="text" name="to_txt" id="to_txt" class="form-control form-control-sm"  data-toggle="tooltip" title="Enter marks" size="2">
                              <span class="text-danger" id="class_error"><?php echo form_error('to_txt'); ?></span>
                            </div>
                          </div> <!-- /row -->
                          <div class="row">
                            <div class="col-lg-1 col-sm-1">
                        ...
                        <?php } ?>
                                <!-- Show button -->
                              <button id="btn_show" name="btn_show" type="button" class="btn btn-primary btn-sm" value="Show"><i class="fa fa-fw fa-eye mr-1"> </i>Show</button>
                            </div>
                          </div>
                          <div class="row">   
                            <div class="col-lg-1 col-sm-1">
            ...
            <div class="modal-body">
              <div class="row">
                <div class="col-sm-12 my-auto">
                  ...
                  4. Grade තේරූ විට පමණක් Subjects දිස්වේ.<br>
                  5. ලකුණු පරාසයක් ලබා දීමේදී from හි ආරම්භක අගය ද to හි අවසාන අගය ද ඇතුළත් කරන්න.<br>
                  6. From ස්ථානය් '0' අතුලත් කිරීමට අවශ්‍ය වූ විට ඒය '00' ලෙස ඇතුලත් කරන්න.<br>
                </div>
              </div>
            </div>
  ...
  <?php } ?> 
        ...
        var marks_from = $('#from_txt').val();
        var marks_to = $('#to_txt').val();

        if( census_id=='' || year=='' || term=='' || grade_id=='' || subject_id=='' || marks_from=='' || marks_to==''){
          swal('Year, Term, Grade, Subject, Marks from, Marks to are required!!!');
        }else if(marks_from < 0 || marks_from > 100){
          swal('Invalid marks near from!!!');
        }else if(marks_to < 0 || marks_to > 100){
          swal('Invalid marks near to!!!');
        }else{
          $.ajax({
            url:"<?php echo base_url(); ?>Marks/listOnRange",
            dataSrc: "Data",
            method:"POST",
            data:{census_id:census_id,year:year,term:term,grade_id:grade_id,class_id:class_id,subject_id:subject_id,marks_from:marks_from,marks_to:marks_to}, 
    ...
    // ශ්‍රේණිය තෝරනවිට subjects ලෝඩ් කිරීමට යොදා ගැනේ. 
    $(document).on('change', '#grade_select', function(){  
      ...
      if(!census_id){
        ...
        $.ajax({  
            url:"<?php echo base_url(); ?>Subjects/viewSubjectsOfAGrade",  
            method:"POST",  
            data:{census_id:census_id,year:year,grade_id:grade_id},  
            dataType:"json",  
            success:function(subjects)  
            {  
              if( !subjects ){
                //swal('Subjects not found!!!!')
                $('select#subject_select').html('');
                $('select[name="subject_select"]').append('<option value="">---No Subjects---</option>').attr("selected", "true");   
              }else{
                $('select#subject_select').html('');
                $('select[name="subject_select"]').append('<option value="">---Select---</option>').attr("selected", "true");
                $.each(subjects, function(key,value) {
                    $('select[name="subject_select"]').append('<option value="'+ value.subject_id +'">'+ value.subject +'</option>');
                });
              }  
            }, 
            error: function(xhr, status, error) {
            ...
            $.ajax({  
                ...
                {  
                  if(!classes){
                    ...
                    $('select#grade_select').html('');
                    $('select[name="grade_select"]').append('<option value="">---Grade---</option>').attr("selected", "true");
                    $('select#class_select').html('');
                    $('select[name="class_select"]').append('<option value="">------</option>').attr("selected", "true");
                    $('select#subject_select').html('');
                    $('select[name="subject_select"]').append('<option value="">------</option>').attr("selected", "true");
                    $.each(classes, function(key,value) {
                      $('select[name="grade_select"]').append('<option value="'+ value.grade_id +'">'+ value.grade +'</option>');
                    }); 
                  }

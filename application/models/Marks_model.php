...
class Marks_model extends CI_Model{
    ...
    // not used yet
    function get_term_test_absentees( $census_id, $year, $term, $grade='', $class='', $index_no='' ){
        $this->db->select('*');
        ...
        $this->db->where('ttat.census_id',$census_id)->where('ttat.year',$year)->where('ttat.term',$term);
        if( !empty( $grade ) ){
            $this->db->where('stgct.grade_id', $grade);
        }if( !empty( $class ) ){
            $this->db->where('stgct.class_id', $class);
        }if( !empty( $index_no ) ){
            $this->db->where('ttat.index_no', $index_no);
        }
    ...
    // check student's marks of any year exists before delete that student by school user
    // used is student controller -> deleteStudent()
    function check_marks_of_a_student_exist($index_no,$census_id){
        $this->db->select('*');
        $this->db->from('term_test_marks_tbl ttmt');
        $this->db->where('ttmt.index_no',$index_no)->where('ttmt.census_id',$census_id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return true;
        } else {
    ...
    function check_marks_completed_in_a_class($census_id,$grade_id,$class_id,$year,$term=""){
        ...
        $this->db->where('ttmct.census_id',$census_id)->where('ttmct.grade_id',$grade_id)->where('ttmct.class_id',$class_id)->where('ttmct.year',$year);
        if(!empty($term)){
            $this->db->where('ttmct.term',$term);
        }
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return true;
    ...
    // used for header notifications, this will get which classes to be updated for this year
    function get_shortage_of_term_test_marks($census_id,$year,$term){
        //echo $census_id.$year.$term;
        $this->load->model('SchoolClass_model');
        $total_classes = $this->SchoolClass_model->get_class_count_school_wise($census_id,$year); // get number of items
        if($total_classes){ // if classes found
            // get marks finalyzed classes
            $this->db->select('*');
            $this->db->from('term_test_marks_confirm_tbl ttmct');
            $this->db->where('ttmct.census_id',$census_id)->where('ttmct.year',$year)->where('ttmct.term',$term);
            $query = $this->db->get();
            //echo $finalized_classes = $query->num_rows(); die();
            $finalized_classes = $query->num_rows();
            if($finalized_classes < $total_classes) {
                // every school must have term test mar details equal to number of items
                // other wise difference must be shown as a notification
                return $total_classes - $finalized_classes;
            } else {
                return false;
            }   
        }else{
    ...
    // used in positionOnMarks in Marks Controller
    function get_position_on_avg_marks( $census_id,$year,$term,$grade_id,$class_id=''){
        // select fields
        $this->db->select('sdt.sch_name,ttrt.index_no as adm_no,st.name_with_initials,ttrt.year,ttrt.term,total,average,grade,class,ttrt.date_updated as last_update');
        $this->db->from('term_test_results_tbl ttrt');
        // joining tables
        $this->db->join('student_tbl st','ttrt.index_no = st.index_no'); // 
        $this->db->join('school_details_tbl sdt','ttrt.census_id = sdt.census_id'); // 
        $this->db->join('student_grade_class_tbl sgct','ttrt.index_no = sgct.index_no'); //
        $this->db->join('grade_tbl gt','sgct.grade_id = gt.grade_id');
        $this->db->join('class_tbl ct','sgct.class_id = ct.class_id');

        $this->db->where('ttrt.census_id',$census_id)->where('ttrt.year',$year)->where('ttrt.term',$term)->where('sgct.grade_id',$grade_id)->where('sgct.year',$year)->where('sgct.census_id',$census_id)->where('st.census_id',$census_id);
        if( !empty($class_id) ){
            $this->db->where('sgct.class_id',$class_id);
        }

        $this->db->order_by('ttrt.average','desc');
    ...
    // used in positionOnMarks in Marks Controller
    function get_position_on_subject_marks($census_id, $year, $term, $grade_id, $class_id='', $subject_id){
        // select fields
        $this->db->select('sdt.sch_name,ttmt.index_no as adm_no,st.name_with_initials,ttmt.year,ttmt.term,subject,ttmt.marks as marks,grade,class,ttmt.date_updated as last_update');
        $this->db->from('term_test_marks_tbl ttmt');
        $this->db->where('ttmt.subj_id', $subject_id);
        // joining tables
        $this->db->join('student_tbl st','ttmt.index_no = st.index_no'); // 
        $this->db->join('school_details_tbl sdt','ttmt.census_id = sdt.census_id'); // 
        $this->db->join('student_grade_class_tbl sgct','ttmt.index_no = sgct.index_no'); //
        $this->db->join('subject_tbl sjt','ttmt.subj_id = sjt.subject_id'); // 
        $this->db->join('grade_tbl gt','sgct.grade_id = gt.grade_id');
        $this->db->join('class_tbl ct','sgct.class_id = ct.class_id');
        // used when admin or zonal search marks in whole zone
        if( $census_id == 'All' ){
            $this->db->where('ttmt.year',$year)->where('ttmt.term',$term)->where('ttmt.subj_id',$subject_id)->where('sgct.grade_id',$grade_id)->where('sgct.year',$year);
        }else{
            $this->db->where('ttmt.census_id',$census_id)->where('ttmt.year',$year)->where('ttmt.term',$term)->where('ttmt.subj_id',$subject_id)->where('sgct.grade_id',$grade_id)->where('sgct.year',$year)->where('sgct.census_id',$census_id)->where('st.census_id',$census_id);
        }
        ...
        $this->db->order_by('ttmt.marks', 'desc');
        $query = $this->db->get();
    ...
    // used in listOnRange() in Marks Controller
    function get_student_list_in_a_range($census_id,$year,$term,$grade_id,$class_id='',$subject_id,$marks_from,$marks_to){
        // select fields
        $this->db->select('sdt.sch_name,ttmt.index_no as adm_no,st.name_with_initials,ttmt.year,ttmt.term,subject,ttmt.marks as marks,grade,class,ttmt.date_updated as last_update');
        $this->db->from('term_test_marks_tbl ttmt');
        $this->db->where('ttmt.subj_id',$subject_id);
        // joining tables
        $this->db->join('student_tbl st','ttmt.index_no = st.index_no','inner'); // 
        $this->db->join('school_details_tbl sdt','ttmt.census_id = sdt.census_id'); // 
        $this->db->join('student_grade_class_tbl sgct','ttmt.index_no = sgct.index_no'); //
        $this->db->join('subject_tbl sjt','ttmt.subj_id = sjt.subject_id'); // 
        $this->db->join('grade_tbl gt','sgct.grade_id = gt.grade_id');
        ...
        if( $census_id == 'All' ){
            //$this->db->where('ttmt.year',$year)->where('ttmt.term',$term)->where('ttmt.subj_id',$subject_id)->where('sgct.grade_id',$grade_id)->where('sgct.year',$year);
            $this->db->where('ttmt.year',$year)->where('ttmt.term',$term)->where('ttmt.subj_id',$subject_id)->where('sgct.grade_id',$grade_id)->where('sgct.year',$year)->where('ttmt.marks >=',$marks_from)->where('ttmt.marks <=',$marks_to);
        }else{
            $this->db->where('ttmt.census_id',$census_id)->where('ttmt.year',$year)->where('ttmt.term',$term)->where('ttmt.subj_id',$subject_id)->where('sgct.grade_id',$grade_id)->where('sgct.year',$year)->where('sgct.census_id',$census_id)->where('st.census_id',$census_id)->where('ttmt.marks >=',$marks_from)->where('ttmt.marks <=',$marks_to);
        }        
